...
> **Liên quan:** [rule_based_genealogy_extraction_steps.md](./rule_based_genealogy_extraction_steps.md), [output_formats_and_ui_plan.md](./output_formats_and_ui_plan.md), [RESEARCH_SOURCES.md](../thesis/RESEARCH_SOURCES.md)
...
**Pilot VGP (Label Studio, Quốc ngữ):** schema cụ thể `PER_NAME` / `GENERATION` / `DATE` / `ORDER` / `LOC` + quan hệ `FATHER_OF` / `MOTHER_OF` / `SPOUSE` — xem [label_studio_pipeline/HUONG_DAN_GAN_NHAN.md](../../research/label_studio_pipeline/HUONG_DAN_GAN_NHAN.md).
